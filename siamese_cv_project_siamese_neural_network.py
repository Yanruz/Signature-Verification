...
class SiameseConvNet(Module):
    def __init__(self):
        ...
        self.cnn = nn.Sequential(
            ...
            Conv2d(1, 48, kernel_size=(11, 11), stride=1),
            nn.ReLU(inplace=True),
            LocalResponseNorm(48, alpha=1e-4, beta=0.75, k=2),
            MaxPool2d(kernel_size=(3, 3), stride=2),
            
            Conv2d(48, 128, kernel_size=(5, 5), stride=1, padding=2),
            nn.ReLU(inplace=True),
            LocalResponseNorm(128, alpha=1e-4, beta=0.75, k=2),
            MaxPool2d(kernel_size=(3, 3), stride=2),
            nn.Dropout(p=0.3),

            Conv2d(128, 256, kernel_size=(3, 3), stride=1, padding=1),
            nn.ReLU(inplace=True),
            Conv2d(256, 96, kernel_size=(3, 3), stride=1, padding=1),
            nn.ReLU(inplace=True),
            MaxPool2d(kernel_size=(3, 3), stride=2),
            nn.Dropout(p=0.3),
...
# Train dataset
train_dataset = GetDataset("train", transform=transforms.Compose([transforms.Resize((220,155)),
                                                                      transforms.ToTensor()
                                                                      ]))
...
# Load test dataset
test_dataset = GetDataset("test", transform=transforms.Compose([transforms.Resize((220,155)),
                                                                      transforms.ToTensor()
                                                                      ]))
